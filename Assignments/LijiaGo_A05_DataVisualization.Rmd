...
NiwotRidge$weighDate <- as.Date(NiwotRidge$weighDate)
NiwotRidge$setDate <- as.Date(NiwotRidge$setDate)
NiwotRidge$collectDate <- as.Date(NiwotRidge$collectDate) 
NiwotRidge$ovenStartDate <- as.Date(NiwotRidge$ovenStartDate)
NiwotRidge$ovenEndDate <- as.Date(NiwotRidge$ovenEndDate)
...
TPvPO4<-
  ...
  labs(x = "Phosphate", y = "Total Phosphorous") +
  xlim(0,30) +
  ylim(0,100)
print(TPvPO4)
...
```{r}
#5(a) Temperature vs month boxplot. 
Temp.monthplot <- 
...
TP.monthplot <- 
  ...
  ylim(0,160) +
  scale_x_discrete(drop=FALSE) +
  labs(x="Month", y="Total Phosphorous (ug)", color="Lake Name")
...
combinedplots <- plot_grid(Temp.monthplot.NL, TP.monthplot.NL, TN.monthplot.NL, nrow=1, align= 'h', rel_heights=c(1.25,1))
print(combinedplots)
...
> Answer: 6 is more effective. While we are able to see the dry mass ranges for each NLCD class more clearly on plot 7, for comparison between dry weights of the different classes, plot 6 is more effective as the data points are all on the same axis. 