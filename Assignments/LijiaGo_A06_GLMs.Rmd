...
> Answer: There is significant difference in mean temperatures among the lakes. This is because the p-value is less than 0.05, and therefore we reject the null hypothesis which states that the mean temperature among lakes are the same. 
...
>Answer: Lakes that have the same mean temperature statistically to Peter Lake are Central Long Lake, Crampton Lake, East Long Lake and Hummingbird Lake, as the lakes in these pairings all have p values < 0.05. From the pairwise groups, we can tell that there are no lakes that have a mean temperature that is statistically distinct from all other lakes. 
...
>Answer: The p value is 0.2649 > 0.05, which means the results are insignificant. This matches the result from part 16, where we find that the Ward Lake-Crampton Lake pair has a p value of 0.999, meaning that we fail to reject the null and did not find that the mean temperatures for the lakes are equal. 
