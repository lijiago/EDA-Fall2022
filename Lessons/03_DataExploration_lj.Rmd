...
# Absolute file path is commented out. working directory is set to the whole folder
#setwd("/home/EDA-Fall2022")
here()
```
...
str(USGS.flow.data) # show dataframe and all objects in dataframe
dim(USGS.flow.data) # how many rows and columns
length(USGS.flow.data) # how many columns

head(USGS.flow.data) # first 6 rows
head(USGS.flow.data, 10) # first 10 rows
tail(USGS.flow.data, 5) # last 5 rows (tail shows bottom of dataframe)
USGS.flow.data[30000:30005, c(3, 8, 14)] #rows 300000-30005, only column 3, 8 and 4

class(USGS.flow.data$datetime) #to figure out type of each column
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #summary of statistics for column. 
...
```{r Removing NAs}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #create new object to store data without NAs
dim(USGS.flow.data) # dim() returns dimensions of dataframe
...
%d  day as number (0-31)  #lower case !! 
...
create.early.dates <- (function(d) {
       ...
       }) #paste0() helps to glue strings together
#
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime) # data still in characters

#
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y%m%d") #transformed back to date 
...
```{r Saving data}
write.csv(USGS.flow.data, file = "./Data/Processed/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE) #stored in the processed data folder
