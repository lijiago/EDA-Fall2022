...
-   **Lists** Ordered collection of elements that can have different modes. #put different things in the same location
...
# Vectors ---- show up as value, not dimensional datasets
...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # values go by the column
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # values go by the row 
matrix2
...
matrix1[4, ] # specify how you want to retrieve row
matrix1[ , 3] # specify how you want to retrieve the column
matrix1[c(12, 14)] # ditching row column coordinates - give 12th and 14th item of the vector
matrix1[c(12:14)] # : means 12 through 14
matrix1[2:4, 1:3] # row and column notation. : is a slice. rows 2-4, columns 1-3

cells <- c(1, 26, 24, 68) #creating a vector
rnames <- c("R1", "R2") 
cnames <- c("C1", "C2") 
...
e <- c("red", "white", "red", NA) # What type of vector? character. NA is a placeholder. df have to have the same number of rows
...
Find a ten-day forecast of temperatures (Fahrenheit) for Durham, North Carolina. Create two vectors, one representing the high temperature on each of the ten days and one representing the low. 2 sets of 10 vales

```{r DataExercise-1a}

low <- c(55, 56, 58,60,61,63,63,64,64,63)
high <- c(81,83,83,84,78,71,79,82,85,84)
dataframe_temp <- data.frame(low, high)
