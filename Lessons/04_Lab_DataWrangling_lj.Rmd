...
```{r}
class(NTL.phys.data$lakeid) #check the class, to know what you are filtering for. 
class(NTL.phys.data$depth)

# dplyr filtering
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0) #store a new dataframe for the surface depths
...
NTL.phys.data.Tues.90thru99 <- filter(NTL.phys.data.Tues & year4>1989 & year4<2000)

NTL.phys.data.90thru99_2 <- 
  NTL.phys.data %>%
...
NTL.phys.data.Ex3 <-
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake" & year4>=1990 & year4<=1999 & month(sampledate) == 7)
...
NTL.phys.data.July.90thru99.surface <- 
  NTL.phys.data.Ex3 %>%
  filter(depth == 0) %>%
