...
library(lubridate)
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE) #stringsAsFactors=TRUE, summary will work for character entries

colnames(NTL.phys.data) #some dates are repeated as there are diff o2 levels and lights at different depths. 
head(NTL.phys.data)
...
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") #format always in quotes
head(NTL.phys.data$sampledate) #to check if it was converted correctly
```

## Data Wrangling
#to process data so that it becomes useful fror you
...
```{r, results = "hide"}
vignette("dplyr") #vignette gives intro to dplyr
```
...
`==` #compare value to specific value
`!=` #check if variables are different
`<`
`<=`
`>`
`>=`
`&` #multiple conditions to check, use &
`|` #if one or the other should be true

```{r}
class(NTL.phys.data$lakeid) #can only compare character with character
class(NTL.phys.data$depth) #can only compare numerics with numerics
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] #observations can be viewed in Environment tab

## how is matrix filtering different from dplyr filtering? (dplyr is more general)
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") #filter data for Paul Lake and Peter Lake
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") #exclude data from these lakes 
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) #use %in% to create a vector for those you variables u want to select, so dont need to keep typing the column name and variable name.

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) #days in the year are numbered from 1-365
...
Arranging allows us to change the order of rows in our dataset. By default, the arrange function will arrange rows in ascending order. (by default) if want to use descending order, need to specify

```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) #arrange(dataset, column you want to arrange by)
...
```{r}
NTL.phys.data.temps <- select(NTL.phys.data, lakename, sampledate:temperature_C) #sample date, depth and temperature are columns next to each other in the dataframe, so can just use the : to specify the first and last column in question
...
NTL.phys.data.temps <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32) #mutate (orginal data set, generate new column name = function of rules that u would like to use )
...
NTL.phys.data.PeterPaul1 <- mutate(NTL.phys.data.PeterPaul1, month = month(sampledate)) #create new column "month". sampledate is a date object

# reorder columns to put month with the rest of the date variables
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments) #select function to change order of column.

# find out the start and end dates of the dataset
interval(NTL.phys.data.PeterPaul1$sampledate[1], NTL.phys.data.PeterPaul1$sampledate[21613]) #create interval object that has specified start and end date
interval(first(NTL.phys.data.PeterPaul1$sampledate), last(NTL.phys.data.PeterPaul1$sampledate)) #for data sets that are not organised by dates (from early to late), better to use this, will work for every dataset of any size
```


## Pipes

Sometimes we will want to perform multiple functions on a single dataset on our way to creating a processed dataset. We could do this in a series of subsequent functions or create a custom function. However, there is another method to do this that looks cleaner and is easier to read. This method is called a pipe. We designate a pipe with `%>%`. A good way to think about the function of a pipe is with the word "then". #using pipe dont need to keep repeating name of dataset
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>% #because using as a pipe, don't need to provide name of dataset inside the function
