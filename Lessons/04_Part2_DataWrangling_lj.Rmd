...
getwd()
library(plyr)
library(tidyverse) #if you have tidyverse, dont need dplyr
library(lubridate)
...
class(NTL.phys.data.PeterPaul$sampledate) #convert back to date object
...
summary(NTL.nutrient.data.PeterPaul$lakename) # Notice that other lake names didn't go away, even though they have zero values
NTL.nutrient.data.PeterPaul <- droplevels(NTL.nutrient.data.PeterPaul) #this removes the names that have zero values
summary(NTL.nutrient.data.PeterPaul$lakename)
summary(NTL.nutrient.data.PeterPaul$comments)
class(NTL.nutrient.data.PeterPaul$sampledate) #still a factor here cos we used "StringAsFactor" function earlier
...
NTL.nutrient.data.PeterPaul <- 
  ...
  mutate(month = month(sampledate)) %>% # mutate month column
  select(lakeid:daynum, month, sampledate:comments) %>% # select to reorder columns
  drop_na(depth) #drop all NAs
#results stored in original dataset under "Environment" tab 

summary(NTL.nutrient.data$depth)
...
#for 'pivot_longer' transforming data into rows so it makes data longer. 
#for 'pivot_wider' transforming data into columns so it makes data wider.
...
NTL.nutrient.data.PeterPaul.gathered <- pivot_longer(NTL.nutrient.data.PeterPaul.subset,tn_ug:po4, names_to = "nutrient", values_to = "concentration") #pivot_longer(original dataset, columns in dataset specified, creating a column named nutrients and whatever that nutrient is will get assigned to that row, 2nd column with concentration of nutrient)

is.na(NTL.nutrient.data.PeterPaul.gathered$concentration[1]) #is.na to check if the row[1] has data or not. false means there is data, true means it is NA
NTL.nutrient.data.PeterPaul.gathered <- subset(NTL.nutrient.data.PeterPaul.gathered, !is.na(concentration)) #remove NAs
count(NTL.nutrient.data.PeterPaul.gathered, nutrient) #count for each nutrient, how many observations we have
...
NTL.nutrient.data.PeterPaul.spread2 <- pivot_wider(NTL.nutrient.data.PeterPaul.gathered, names_from = nutrient, values_from = concentration) #convert back nutrients to column

# Split components of cells into multiple columns
# Opposite of 'separate' is 'unite'
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.subset, sampledate, c("Y", "m", "d")) #separate columns for year month and date

NTL.nutrient.data.PeterPaul.dates2 <- unite(NTL.nutrient.data.PeterPaul.dates, sampledate, c("Y","m","d")) #unite the columns back together
...
#if some columns match but some don't match, cannot use rbind. but can just join
...
NTL.phys.nutrient.data.PeterPaul <- full_join(NTL.phys.data.PeterPaul.subset, NTL.nutrient.data.PeterPaul.subset) #joining by common columns in dataset
...
```{r}
LitterFiles = list.files(path = "./Data/Raw/NIWO_Litter/", pattern="*.csv", full.names=TRUE) #specify folder to where all the files u want to combine are loaded, pattern = files u want to read
LitterFiles #LitterFiles has all the folders u want to read

Litter <- LitterFiles %>%
  plyr::ldply(read.csv) #if got error "could not find function" means that you didnt load the package earlier
```
...
LitterTrap <- left_join(Litter, Trap, by = c("siteID", "plotID", "trapID")) #same number of observations as litter, as its the longest data set. 
...
NTL.PeterPaul.summaries <- 
  ...
  filter(depth == 0) %>% #want surface info only
  group_by(lakename, month) %>% #group by lakename and month
  filter(!is.na(temperature_C) & !is.na(tn_ug) & !is.na(tp_ug)) %>% #filter NAs for this 3 columns. for this 3 columns, dont want NAs. want a row where none of this 3 are NAs
  summarise(meantemp = mean(temperature_C), 
            sdtemp = sd(temperature_C), 
            meanTN = mean(tn_ug), 
            sdTN = sd(tn_ug), 
            meanTP = mean(tp_ug), 
            sdTP = sd(tp_ug)) #all the summaries u want

NTL.PeterPaul.summaries

write.csv(NTL.PeterPaul.summaries, row.names = FALSE, 
          file ="./Data/Processed/NTL-LTER_Lake_Summaries_PeterPaul_Processed.csv") #saved summaries in a processed file 
