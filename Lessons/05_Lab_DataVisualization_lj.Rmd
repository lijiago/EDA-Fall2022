...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(x=0,y=0.025,label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))) #use function expression when you want to write an equation. paste() is to add default elements together
print(Plot_text)
...
ggplot(PeterPaul.chem.nutrients, aes(x=month)) + geom_bar() 

PeterPaul.chem.nutrients$month_f <- factor(PeterPaul.chem.nutrients$month, levels=1:12, labels=month.abb)
  
ggplot(PeterPaul.chem.nutrients, aes(x=month_f)) + geom_bar() + scale_x_discrete(drop=F)
...
# Change x axis to include marker/labels every 5 years
  ...
  #bad example. need to put color=depth and size 0.5 within aes() of geom_point so that r knows which points you want to change the size 
ex2 <-
...
ex2b <- 
  ...
  scale_x_date(
    date_breaks = "5 years", date_labels = "%Y")
print(ex2b)
 #scale_x_ -> depends on class of the x column
