...
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray() #theme_gray is default for ggplot
print(O3plot1)

O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_bw() #classic dark on light theme. background is white
print(O3plot2)
...
* axis label colors #default black and gray
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #lty=2 is a dashed line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) +
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #can add text anywhere on the plot. just need to specify where  # hjust = 1 , is the margin alignment and 1 means left align
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #specify limits on x axis, just specify first and last date
    date_breaks = "2 months", date_labels = "%b %y") + 
  ylab(expression("O"[3]* " AQI Value")) + #change to more informative label, expression is for O3 subscript for 3
...
scales::show_col(colormap(colormap = colormaps$viridis, nshades = 16)) #show colours for colour map
...
NvsP2 <-
  ...
  scale_shape_manual(values = c(15, 17)) + #change shape of points
