...
#r2 for linear. measure of percentage of variability in y explained by the independent variable. r2 close to 1 almost all variability in y explained by x. if closer to 0, not all the variability is explained by x
...
```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) #lm is linear model
...
#p values <0.05 (coefficients are statistically different than 0, worthwhile to estimate irradience in lake based on that information)
#results of regression tell us we did a meaningful regression

# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth)
#negative correlation. increase depth, decrease light level. strength of correlation is 0.55, strong correlation
```
...
-> we need to check the residuals for regressions analysis thru plot()
...
1. **Residuals vs. Fitted.** The value predicted by the line of best fit is the fitted value, and the residual is the distance of that actual value from the predicted value. By definition, there will be a balance of positive and negative residuals. Watch for drastic asymmetry from side to side or a marked departure from zero for the red line - these are signs of a poor model fit. -> scatter plot.

2. **Normal Q-Q.** The points should fall close to the 1:1 line. We often see departures from 1:1 at the high and low ends of the dataset, which could be outliers. -> compare residuals with normal distribution, and if they follow normal dist, points should fall close to diagonal line of qq plot

3. **Scale-Location.** Similar to the residuals vs. fitted graph, this will graph the squared standardized residuals by the fitted values. -> similar to residual vs fitted. 
...
par(mfrow = c(2,2),  mar=c(4,4,4,4)) #dividing screen into 4 plot cell with 2x2 grids
plot(irradiance.regression) #for first plot, want the line to be as flat as possible; 3rd plot look at symmetry, 4th plot look at outliers. those that fall outside dashed lines are the ones that are outliers
par(mfrow = c(1,1))
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #ylim to change plotting window to show values between 0 to 2000
  geom_point() 
print(irradiancebydepth) 

#scatter plot is not linear, can log to make linear
```
...
irradiancebydepth2 <- 
  ...
  scale_y_log10() + #y axis is log transformed irradiance water
  geom_point() 
...
## Multiple Linear Regression -> more than 1 variable on right side (more than 1 x)
...
#these are residual plots for the regression
par(mfrow = c(2,2), mar=c(4,4,4,4))
plot(TPregression)
par(mfrow = c(1,1))
#for residuals vs fitted, look for symmetry wrt the red line, and also want red line to be flat. this is not a good residual series
#normal-qq not too bad
# scale-location, same prob as residuals vs fitted
#residuals vs leverage, some look like they are outliers
...
## Correlation Plots
We can also make exploratory plots of several continuous data points to determine possible relationships, as well as covariance among explanatory variables. clear from r2, theres something causing variability. 
# looks at correlation coeff. create correlation matrix to see correlation coeff for all the datasets
...
PeterPaulCorr <- cor(PeterPaulnutrients) #find variables that are highly correlated with phosphorous
corrplot(PeterPaulCorr, method = "ellipse") #thinner the scatter plots, the higher the correlation. changing from dark red to dark blue. strong neg to strong positive correlation. symmetric matrix, whatever u have on 1,2, you will have on 2,1
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #print upper diagonal only. shows value of ellipses as well
```
...
Paul.naomit <- PeterPaul.chem.nutrients %>%
  ...
  na.omit()
#consider all nutrients as explanatory variables to explain total phosphorous concentration. so total p is a function of depth, dissolved o2, temp, tn and phosphate. 
TPAIC <- lm(data = Paul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4)

#Choose a model by AIC in a Stepwise Algorithm. higher the AIC value, worse the model fit. analyses removal of each variable one by one.
step(TPAIC)
