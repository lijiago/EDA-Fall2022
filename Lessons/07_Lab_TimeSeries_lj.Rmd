...
#rename as rainfall, separating (same as text to columns in excel, using space to separate. space before month and after month in the original data set)
#need to paste month and year together so that the date can be parsed
...
#no NAs
#create new vector -> use left join to compare data and find missing 
...
Rainfall_Time <- ggplot (Rainfall_BR_processed, aes(x=Date, y=Rainfall)) +
  geom_line() +
  geom_smooth(method = "lm", se=FALSE)
print(Rainfall_Time)
# trend in rainfall? cant say much about trend. seems like slope is about 0 (stationary)

Temperature_Time <- ggplot(Temp_BR_processed, aes(x=Date, y=Temperature_C)) +
  geom_line() +
  geom_smooth(method = "lm", se=FALSE)
print(Temperature_Time)
#increasing trend in temperature. (positive trend, average temp increasing over time)
...
plot(Rainfall_TS)
Temperature_TS <- ts(BRA_complete$Temperature_C, start=c(f_year, f_month), frequency = 12)
plot(Temperature_TS)
...
Rainfall_TS.decomposed <- stl(Rainfall_TS, s.window="periodic")
plot(Rainfall_TS.decomposed)

#seasonal is explaining alot of variability of dataset. based on the bars on the right

Temperature_TS.decomposed <-stl(Temperature_TS, s.window="periodic")
plot(Temperature_TS.decomposed)
#trend is as important as seasonality when decomposing dataset.
...
BR_Rain_components <- as.data.frame(Rainfall_TS.decomposed$time.series[,1:3]) #<- transforming timeseries backt o data frame

BR_Temp_components <-as.data.frame(Temperature_TS.decomposed$time.series[,1:3])

#add back date to dataframe
...
#trend is small part of time series data. 

#seasonal component is not going to be in line with the rest of the data set. not ideal visualisation, but just trying to represent the components we have here. 
#subtract mean from overall to push the entire graph down to map seasonality onto the overall graph. 
