...
huc8_sf_UN_woUN <- st_read('./Data/Spatial/NCHUC8.shp') %>%
  filter(SUBBASIN!='Upper Neuse')


#Create a map
#install.packages('leaflet.extras2')
mapview(huc8_sf, col.regions='orange') | mapview(huc8_sf_UN, col.regions='purple', alpha.regions=1) 
mapview(huc8_sf, col.regions='orange') + mapview(huc8_sf_UN, col.regions='purple', alpha.regions=1, layer.name = 'HUC 8')

#using ggplot

ggplot() + geom_sf(data = huc8_sf, fill= 'orange' ) + geom_sf(data=huc8_sf_UN, fill = 'purple')
...
HUC8wind <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', 
...
huc8_sf2 <- merge(x=huc8_sf,
                  y=HUC8wind, #merge y data into x data
                  by.x= 'HUC_8',
                  by.y= 'HUC_8')

#View the outputs
mapview(huc8_sf2, zcol='kWhkm2day')
