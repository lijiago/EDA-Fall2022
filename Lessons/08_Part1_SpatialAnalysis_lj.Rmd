...
## 2.1 Simple feature geometry (sfg) -> sequence of coordinate pairs. geometry representing one feature we want to show on the landscape
...
-**vertex** anytime it changes direction
...
## 2.4 Coordinate reference systems (crs) -> enables us to pin the feature on earth
...
A **spatial dataframe**, also referred to as **simple features** (or `sf`), **spatially enabled dataframe**, or **geodataframe** is a collection of simple features (geometry + attribute + crs) stored in the format of a dataframe. It inherits all the capabilities of a standard dataframe but also has spatial analysis capabilities. Each row is a feature
...
#web resource. st_read to read the geoJson format and store it as GasPlants. pointing straight to a website instead
...
#View using ggplot
#geom_sf spatial dataframe. points in geographic space, lat and long. follows shape of NC
ggplot(data=GasPlants) + geom_sf()

#View using mapview
#pulls in attributes as well, can play around with backdrops
mapview(GasPlants)
...
#lat long values can be used for creating geometry. 

#got alot of attributes for same sites
...
#st_as_sf : simple table as simple feature
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), #tell LONG LAT is (x,y)
           crs=4326)

mapview(EPAair.sf)
#lat long gets collapsed into one column
...
#simple feature point: sfg. 
sfg_GraingerHall <- st_point(c(-78.9427,36.00465))
class(sfg_GraingerHall)
mapview(sfg_GraingerHall)
#map view shows point, but does not know where the coordinate links to earth

#Upgrade to a simple feature geometry column by adding a crs (coordinate reference system)
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326)
class(sfc_GraingerHall)
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe (simple feature table)
sf_GraingerHall <- st_as_sf(sfc_GraingerHall)
sf_GraingerHall$Name = 'Grainger Hall' #attaching attributes to construct a spatial dataframe from scratch
...
Lat long is good to locate things on maps, but not good to measure distances. Need to convert to cartesian coordinates (this is known as projection)
...
#take a feature and expand it outward at a certain unit of measure
#1 refers to 1 degree lat and 1 degree long. 
the_buffer <- sf_GraingerHall %>% st_buffer(1)
#need to reconfigure
#projecting data: sphere to planar surface. there will be distortion at some points
#earth is not a perfect sphere
#how to minimise distortion? come up with multiple models
...
The "4326" in the `crs = 4326` is an EPSG code, a number used to look up a specific crs. (a database of coordinate reference systems. can find it on epsg.io webpage)
...
(when working with North America data)
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) #project to 32617 : utm zone 17 N. st_transforms tranforms and reproject to use the 32617 coord system.
GasPlants.utm <- st_transform(GasPlants,32617)

#Have a look at the coordinate associated with the geometries
EPAair.sf.utm$geometry[1] #to confirm that the units are in meters and not angular units. thats why the number is so big

#Plot the data
ggplot() +
  geom_sf(data = EPAair.sf.utm,color='blue') + 
  geom_sf(data = GasPlants.utm,color='red',shape='O') 
#axes still in degrees, but since its projected, there is abit of distortion in the lines


#calculate mean distance to gas sites
#Create a distance matrix between the point in each dataset
distance_matrix <- st_distance(EPAair.sf.utm, GasPlants.utm, pairwise=T) #distance between 2 sets of geometries, pairwise to create a matrix

#Compute the mean of each row (each EPA site)
mean_distances <-  rowMeans(distance_matrix) #reduce dataset to just the means

#Add as a column to the epa sites
#bind distances to epaair.utm
