...
epa_pm25_sites_sf <- read_csv('./Data/Raw/EPAair_PM25_NC2018_raw.csv') %>% #read_csv is a tidy version
  group_by(`Site Name`, COUNTY, SITE_LATITUDE, SITE_LONGITUDE) %>% #one site per location
  summarize(
    meanPM = mean(`Daily Mean PM2.5 Concentration`,na.rm=T),
    maxPM = max(`Daily Mean PM2.5 Concentration`,na.rm=T)
    ) %>% 
  st_as_sf(coords = c('SITE_LONGITUDE','SITE_LATITUDE'), crs=4269) #converts it to a spatial feature object (ie a spatial dataframe)
...
st_crs(epa_pm25_sites_sf) #you see degrees as the units which means the data is spherical, not projected and it belongs on a sphere
...
#geometry, attributes and crs : 3 basic things to create a spatial dataframe
```
...
# st_read instead of read.csv to read a shape file. 
...
#any st_ is a spatial command
#creates a bounding box around the feature and tells the coordinates of the spatial extents of the objects
st_bbox(counties_sf)
...
#polygon dataset, zcol colours based on relative amount of water
...
huc8_sf_UN<- st_read('./Data/Spatial/NCHUC8.shp') %>% 
  filter(SUBBASIN == 'Upper Neuse')

#Create a map

mapview(huc8_sf, col.regions='orange') + mapview(huc8_sf_UN, col.regions='purple')
...
GeoJSON is constructed to share data over the web. 
...
```{r load.geojson.data}
powerplants_sf <- st_read('https://opendata.arcgis.com/datasets/ee0263bd105d41599be22d46107341c3_0.geojson') #reading this does not leave it on the computer's memory, cos its fetching from the net
...
#reporting the number in each category
nc_powerplants_sf %>% 
  st_drop_geometry() %>% 
...
#can see outline of north carolina
...
The data we'll add here is the CDC's Social Vulnerability Index data. (composite of a number of different variables, applying some of their rules to come up with social vulnerability index) Information on this dataset is available here:
...
#data doesnt live on a harddrive, its a webaddress
#purely a csv dataset
svi2018_nc_raw <- read.csv(
  'https://svi.cdc.gov/Documents/Data/2018_SVI_Data/CSV/States_Counties/NorthCarolina_COUNTY.csv',
  colClasses = c('FIPS' = 'factor')) %>% #FIPS is a number, but its a factor, discrete data, if you bring it in as a number, you will lose the leading 0 in the dataset (17 instead of 017, 18 instead of 018)
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)

#Check structure
str(svi2018_nc_raw)

#Join the SVI attributes to the county spatial features because the svi_2018_nc_raw does not have a geometry column
...
mapview(counties_sf_join, 
        zcol = 'E_POV', #coloured by ppl in poverty
        col.regions = brewer.pal(2, 'RdBu')) +  #specify colour ramp with 'RdBu'
  mapview(epa_pm25_sites_sf, cex = 'maxPM') #epa sites, sized by maximum particulate matter
...
windenergy_huc8 <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv', colClasses = c('HUC_12'='factor')) %>%
  mutate(HUC_8=substr(HUC_12,1,8)) %>% #did not have huc8 column, so need to create a new one. specify string from first to 8th value
  group_by(HUC_8) %>%
  summarize(kWhkm2day=sum(kWhkm2day))

#join to HUC_8 features

windenergy_sf <- merge(x=huc8_sf,
                       y=windenergy_huc8,
                       by.x = 'HUC_8',
                       by.y='HUC_8')
...
mapview(windenergy_sf, zcol='kWhkm2day')
...
#Convert all to UTM Zone 17 (crs = 26917) so that everything is in meters
...
#grab all counties that intersect the Neuse polygon
#Select intersecting counties using matrix subsetting
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] 
#get 23 that intersect with neuse_sf

#Select intersecting counties using the `filter()` command
neuse_intersect_2 <- counties_sf_utm %>% 
  filter(st_intersects(x = ., y = neuse_sf, sparse = FALSE)) #apply the st_intersects command, y is the feature/geometry you want to use to select the county. sparse gives a full matrix(?)

#View the result
myMap + mapview(neuse_intersect_2, alpha.regions = 0)
#overlaid 2 geometries, so that we grab whatever that intersected with the upper neuse basin 

#Actually intersect the counties (not just select those that intersect)
neuse_counties_sf <- neuse_sf %>% #cookie cutter
  st_intersection(counties_sf_utm) 
myMap + mapview(neuse_counties_sf, alpha.regions = 0)
#note that the area values dont update now that there are insections 
...
#negative values buffer the inside
...
```{r Vsualizing multiple dataset with ggplot}
#Wrong order
ggplot()  +
  geom_sf(data = epa_sf_utm, color='white', size=2) +
  geom_sf(data = counties_sf_utm, aes(fill = ALAND), color = 'white')  +
  geom_sf(data = state_sf_utm, color='red',size=2) + #state boundary masks everything else
...
library(leaflet)
#must use wgs84 when using Leaflet
