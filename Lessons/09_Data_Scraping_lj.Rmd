...
Web address is a request. address of who will receive the command (google). parameters of request= "nicholas+school"
response: ctrl + U 
...
webpage 
#html made up of tags and values. 
```
...
the_year <- 2014
the_scrape_url <- paste0(the_base_url, '/', the_facility, '/', the_year) #paste0 eliminates the space when you paste the individual items
...
df_withdrawals <- data.frame("Month" = rep(1:12), #produces a sequence of numbers
                             "Year" = rep(the_year,12), #12 repeats 
                             "Avg_Withdrawals_mgd" = as.numeric(the_withdrawals)) %>% 
  mutate(Registrant = !!the_registrant, #!! dont set to a string, refers to a variable
         Facility_name = !!the_facility_name,
         Facility_type = !!the_facility_type,
         Date = my(paste(Month,"-",Year))) #convert month and year to date
...
# 2 inputs, the year and facility
scrape.it <- function(the_year, the_facility){
  ...
  #Sys.sleep(1) #uncomment this if you are doing bulk scraping! tells r to wait for 1 second. will be 1 second slower. reason for that is because we will be reiterating. the server might be DNOS attack. bad etiquette. website provider may cut off access. adding a brief 5 seconds can be helpful
...
#run scraipe.it for every value in years vector (the_years)
#repeat scrape it for every value in the_years. but also specify the_facility, being the specific facility. can copy paste 0004-0001 as well 
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it,
                  the_facility=my_facility)

#OR, use purrr's map function
#mapping values on function. has nothing to do with geospatial mapping
the_dfs <- map(the_years,scrape.it,the_facility=my_facility)

#Conflate the returned dataframes into a single dataframe
#bind_rows in tidyverse version of r_bind
...
ggplot(the_df,aes(x=Date,y=Avg_Withdrawals_mgd)) + 
  geom_line() + 
  geom_smooth(method="loess",se=FALSE) +
  labs(title = paste("2020 Water usage data for",the_registrant), #this part is wrong cos plot shows 2018-2020
       subtitle = the_facility_name,
       y="Withdrawal (mgd)",
       x="Date")
```
...
the_facility_ids <- the_main_website %>% 
  ...
  html_attr("href") %>%           #Pull the "href" attribute from the node (html_attribute) -> you want the attribute associated wth the tag
...
#scrape sites and years at the same time
the_facilities <- sample(the_facility_ids,3)
the_years = c(2018,2019,2020) #if use map2 here, it will pull site 1 2018 data, site 2 2019 data, site 3 2020 data. but we want 3 samples from 3 years. so 9 values in total
...
the_df <- cross2(the_years,the_facilities) %>%  #cross2 generates a list of list.
  map(lift(scrape.it)) %>% #lift. applying it repeatedly to scrape.it, lists unique combinations of all that we want. scrapes all 9 combinations
  bind_rows() #binding with dataframe
