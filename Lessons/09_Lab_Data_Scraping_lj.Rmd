...
#address of server: waterdata.usgs.gov
#nwis/uv: ? 
#parameters: site number, 7 days, format rdb
#5s : string
#20 character date 
...
#Set the URL. change the period to 24 for 24 days within the URL itself
EnoURL <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

#Get the data, which starts on line 30
gage_data_eno <- read.table(EnoURL,skip = 29,header=TRUE,sep='\t',stringsAsFactors = T) 

#Update the column headers
colnames(gage_data_eno) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89062_00060_cd", "gage_ht_ft", "89063_00065_cd")
#Tidy the data
gage_data_eno <- gage_data_eno %>% 
  select(datetime, discharge_cfs,gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

#Plot
ggplot(gage_data_eno,aes(x=datetime,y=discharge_cfs)) + 
  geom_line() +
  labs(x = "Date",y = "Discharge (cfs)")
```
...
#from USGS. specify 1 variable as the siteNo. need to figure out perimeter codes for. (cfs-discharge code)
#(scolre- statistic for mean)
...
```{r}
#Apply the API key
census_api_key("d4ad7874c996c7a83fec0a81466309ddc07d8e22", overwrite=T, install = TRUE)

nc_pop <- 
  get_acs(geography = "county",
          variables = "B01003_001", #should be population
          state = "NC",
          geometry = TRUE)
...
df_electricity <- data.frame(
  state= the_states,
  price=the_price,
  capacity=the_capacity,
  netgeneration=net_generation,
...
scrape.it <- function(the_year){
  ...
  the_url <- ifelse(the_year==2021,
                    'https://www.eia.gov/electricity/state/',
                    paste0('https://www.eia.gov/electricity/state/archive',the_year,'/')) #paste to combine snippets of string together 
...
big_df <- map(seq(2015,2018), scrape.it) %>% bind_rows()
